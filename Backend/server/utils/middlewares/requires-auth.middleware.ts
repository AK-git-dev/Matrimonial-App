  ...
  try {
    ...
    if (!accessToken)
      throw new createHttpError.BadRequest(
        `Access token not found! May be you're logged out!`
      );

    const verifiedToken: TokenInterface = await verifyAuthTokenValidity(
      accessToken
    );
    if (!verifiedToken.userId || !verifiedToken.phoneNumber)
