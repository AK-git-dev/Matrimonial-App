...
const generateOtpCode = (): string => randomBytes (3).toString ('hex');

export function generateOtpAndToken(phoneNumber: string): { otp: string, otpToken: string } {
    const otp: string = generateOtpCode ();
    const otpToken = sign ({phoneNumber , otp} , SECRET , {expiresIn :'120s'});
    return {otp , otpToken};
}
