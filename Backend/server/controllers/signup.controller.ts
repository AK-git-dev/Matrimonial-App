...
router.post(
  ...
  async (req: RequestInterface, res: ResponseInterface, next: Next) => {
    async function checkPhoneNumberAlreadyExists(
      payload: PhoneNumberSignupInterface
    ) {
      ...
      if (isPhoneNumberRegisted !== null)
        throw new createError.Conflict(
          "You have an registered account already! Please Sign in!"
        );
    }
    ...
    try {
      ...
      const ifTokenGenerated = await generateOtpAndTokenHash(
        payload.phoneNumber
      );
