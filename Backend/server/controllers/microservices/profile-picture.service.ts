...
async function handleFileUpload(
    ...
    try {
        ...
        const files: FileUploadInterface[] = (req as any).files.files as FileUploadInterface[];

        const noOfPhotos: number = await Schema.ProfilPicture.count ({where :{UserId}});
        if (noOfPhotos > 10) throw  new createHttpError.Forbidden (`you can not upload more than 10 profile pictures!`);

        if (!files) {
            throw new createHttpError.Forbidden ("No files has been attached!");
        } else if (files.length < 2)
            throw new createHttpError.Forbidden (
                "Please select at least 2 photos of yours"
            );
