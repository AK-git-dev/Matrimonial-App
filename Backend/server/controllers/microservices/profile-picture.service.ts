...
async function handleFileUpload(
  ...
  try {
    ...
    const files: FileUploadInterface[] = (req as any).files
      .files as FileUploadInterface[];

    const noOfPhotos: number = await Schema.ProfilPicture.count({
      where: { UserId },
    });
    if (noOfPhotos > 10)
      throw new createHttpError.Forbidden(
        `you can not upload more than 10 profile pictures!`
      );
