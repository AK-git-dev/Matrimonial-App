...
router.post ('/add-to-favourites/:profileId' , requiresAuth , async (req: RequestInterface , res: ResponseInterface , next: Next) => {
    try {
        const UserId: string = (req as any).userId;
        const profileId: string = (req).params.profileId;

        const userExists = await Schema.User.findByPk (profileId);
        if (userExists === null) throw new createHttpError.BadRequest ('Interest person has some privacy options enabled!');
        ...
        if (alreadyAddedToFavourite)
            throw new createHttpError.BadRequest (
                `You have already added ${userExists.getDataValue ('fullname')} in your favourites`);
...
router.post ('/remove-from-favourites/:profileId' , requiresAuth , async (req: RequestInterface , res: ResponseInterface , next: Next) => {
    try {
        const UserId: string = (req as any).userId;
        const profileId: string = (req).params.profileId;

        await Schema.FavouritePerson.destroy ({where :{favouritePersonId :profileId , UserId}});
