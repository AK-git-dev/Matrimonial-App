...
router.post ('/with-otp' , async (req: RequestInterface , res: ResponseInterface , next: Next) => {
    try {
        ...
        const isUserExists = await Schema.User.findOne ({where :{phoneNumber}});
...
router.post (
    ...
    async (req: RequestInterface , res: ResponseInterface , next: Next) => {
        async function checkPhoneNumberAlreadyExists(phoneNumber: string) {
            const isPhoneNumberRegisted = await Schema.User.findOne ({where :{phoneNumber}})
            if (isPhoneNumberRegisted === null) throw new createError.Conflict ('Seems, You do not have any account! Please Sign up first!');
            return isPhoneNumberRegisted;
        }

        try {
            const {phoneNumber} = req.body as { otpCode: string, phoneNumber: string }; // payload
