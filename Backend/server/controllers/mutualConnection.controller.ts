...
router.get ('/' , requiresAuth , async (req: RequestInterface , res: ResponseInterface , next: Next) => {
    try {
        ...
        const favoritePersonIds: string[] = (await Schema.FavouritePerson.findAll ({
            where :{UserId} ,
        ...
        for (const fid of favoritePersonIds) {
            mutualMatches.push (await Schema.PersonWhoFavouritedHimself.findOne ({
                where :{personWhoFavoritedYouID :fid} ,
                include :[{model :Schema.User , as :"details" , attributes :["id","fullname" , "age" , "gender"] ,}]
            }));
        }
